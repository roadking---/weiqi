...
(function() {
  var find, find_block, move, move_step, repeal;
  ...
  find_block = function(items, pos) {
    var adjacent, block, item, newly_found, opposite, vacant;
    ...
    while (newly_found.length) {
      adjacent = _.chain(newly_found).map(function(x) {
        ...
        return _.chain([block, vacant, opposite]).flatten().some(function(y) {
      ...
      if (adjacent.length) {
        ...
        vacant.push(_.filter(adjacent, function(x) {
    ...
    return {
      block: block,
      vacant: _.chain(vacant).flatten().pluck('pos').groupBy(function(x) {
        return x.join(',');
  ...
  move_step = function(items, step) {
    ...
    if (last && taken.length === 1 && taken[0].length === 1) {
      ...
      if (taken.player === last.player && _.every([0, 1], function(i) {
        ...
        if (taken = last.taken) {
          if (taken.length === 1 && taken[0].length === 1) {
            taken = taken[0][0];
