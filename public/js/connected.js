...
(function() {
  var check_tap_enabled, gid, tap_enabled, _ref,
  ...
  check_tap_enabled = function(e) {
    if (!e) {
      return tap_enabled = false;
    }
    if (e.type === 'tap') {
      tap_enabled = true;
    }
  ...
  $.get("/json/connected/" + gid, function(data) {
    ...
    return $(function() {
      ...
      Game = Backbone.Model.extend({
        ...
        role_of_player: function(some_uid) {
          return _.invert(this.get('seats'))[some_uid];
        },
        my_role: function() {
          if (this.is_player() && this.get('seats')) {
            return this.role_of_player(this.myself());
          }
        },
        opponent: function() {
          if (this.is_player()) {
        ...
        connect: function(cb) {
          ...
          return init_socket(function() {
            _this.socket.on('start', function(seats, next) {
              _this.set('seats', data.game.seats = seats);
              _this.set('next', data.game.next = next);
              return _this.set('status', data.game.status = 'started');
            });
            _this.socket.on('attend', function(player, params) {
              ...
              _this.set('players', data.game.players);
              if (params != null ? params.start : void 0) {
                _this.set('seats', data.game.seats = params.seats);
                _this.set('status', data.game.status = 'started');
                return _this.set('next', _this.get('next'));
              } else if (_this.get('players').length === _this.get('player_num')) {
                return _this.set('status', 'taking_seat');
              }
            });
            ...
            _this.socket.on('call_finishing', function(msg) {
              var analysis, player, rlt, stone_in_regiment, suggest;

              switch (msg) {
                case 'ask':
                  msg = arguments[0], player = arguments[1];
                  return _this.trigger('call_finishing_ask_received', player);
                case 'cancel':
                  msg = arguments[0], player = arguments[1];
                  return _this.trigger('call_finishing_cancel_received', player);
                case 'reject':
                  msg = arguments[0], player = arguments[1];
                  return _this.trigger('call_finishing_reject_received', player);
                case 'accept':
                ...
                case 'confirm':
                  msg = arguments[0], player = arguments[1], rlt = arguments[2];
                  if (rlt) {
                    return _this.trigger('player_confirmed_ending', rlt);
                  }
              }
            });
        ...
        retract: function(direct) {
          ...
          if (direct) {
            if (stone = (_ref1 = stone_list.last()) != null ? _ref1.toJSON() : void 0) {
              stone_list.pop();
              stone_list.chain().filter(function(x) {
                return x.get('repealed') === stone.n;
              }).each(function(x) {
                return x.unset('repealed');
              });
              game.set('next', stone.player, {
                silent: true
              });
              return this.trigger('retract', stone);
            }
          } else {
            if (this.is_player() && this.my_role() === ((_ref2 = stone_list.last()) != null ? _ref2.get('player') : void 0)) {
        ...
        taking_seat: function(seat) {
          var _ref1,
            _this = this;

          if ('taking_seat' === this.get('status') && this.myself() && (_ref1 = this.myself(), __indexOf.call(this.get('players'), _ref1) >= 0)) {
            return this.test_connection(function() {
              return _this.socket.emit('taking_seat', seat, function(res) {
        ...
        call_finishing_confirm: function(cb) {
          var _this = this;

          return this.test_connection(function() {
            return _this.socket.emit('call_finishing', 'confirm', cb);
          });
        },
        initialize: function() {
          var _this = this;
      ...
      CommentList = Backbone.Collection.extend({
        ...
        initialize: function() {
          ...
          return this.listenTo(game, 'change:next', function() {
            ...
            if (!game.get('try')) {
              n = stone_list.length ? stone_list.last().get('n') : 0;
      ...
      BackBtn = Backbone.View.extend({
        ...
        initialize: function() {
          ...
          this.listenTo(game, 'change:show_first_n_stones', this.render);
          return this.start_from = !game.get('contract').rangzi || game.get('contract').rangzi === 'none' ? 0 : Number(game.get('contract').rangzi) - 1;
        },
        events: {
          click: 'click',
          tap: 'click'
        ...
        render: function() {
          ...
          n = game.get('show_first_n_stones');
          if (n && n > this.start_from || n === null && stone_list.length > this.start_from + 1) {
      ...
      BeginningBtn = Backbone.View.extend({
        ...
        click: function(e) {
          ...
          game.set('show_first_n_stones', this.start_from);
          if (game.get('try')) {
            return game.set('next_trying', (stone_list.at(this.start_from).get('player') === 'black' ? 'white' : 'black'));
          }
        },
        render: function() {
          if (stone_list.length > this.start_from + 1 && (game.get('show_first_n_stones') === null || game.get('show_first_n_stones') > this.start_from)) {
            this.$el.removeClass('invalid');
          } else {
            this.$el.addClass('invalid');
          }
      ...
      BoardView = Backbone.View.extend({
        ...
        initialize: function() {
          ...
          this.listenTo(game, 'call_finishing_stop_received', this.clear_style);
          if (game.get('status') === 'taking_seat') {
            return this.$el.addClass('gray');
          }
        },
        events: {
        ...
        click_board: function(e) {
          ...
          if (game.get('show_first_n_stones') && !game.get('try')) {
            ...
            this.show_analysis();
            return $('#finishing #regiments li').removeClass('selected');
          } else if (e.button === 0) {
      ...
      BulletinView = Backbone.View.extend({
        ...
        initialize: function() {
          ...
          this.listenTo(game, 'call_finishing_ask', function() {
            return _this.show_tpl('call_finishing_ask');
          });
          this.listenTo(game, 'player_confirmed_ending', function(rlt) {
            return _this.show_tpl('status_ended', {
              result: rlt
            });
          });
          ...
          this.listenTo(game, 'call_finishing_cancel_received', function(player) {
            return _this.show_tpl('call_finishing_cancel_received', {
              player: player
            });
          });
          this.listenTo(game, 'call_finishing_reject_received', function(player) {
            if (game.is_player()) {
              return _this.show_tpl('call_finishing_reject_received');
            } else {
              return _this.show_tpl('call_finishing_reject_received_others', {
                player: player
              });
            }
          });
          ...
          this.listenTo(game, 'change:status', function(m, v) {
            console.log(v);
            switch (v) {
              case 'started':
                return _this.next_player(game.get('next'));
              case 'taking_seat':
                return _this.show_tpl('taking_seat');
            }
          });
          this.listenTo(regiment_list, 'change:judge', this.show_analysis_confirm);
          if (((_ref1 = game.get('status')) === 'init' || _ref1 === 'taking_seat') && ((_ref2 = game.get('players')) != null ? _ref2.length : void 0) === 1) {
            if (game.myself() === game.get('players')[0]) {
              return this.show_tpl('init_waiting');
            } else {
              return this.show_tpl('init_attending');
            }
          } else if (game.get('status') === 'taking_seat') {
            if (game.myself() && (_ref3 = game.myself(), __indexOf.call(game.get('players'), _ref3) >= 0)) {
              return this.show_tpl('taking_seat');
            } else {
              return this.show_tpl('taking_seat_others');
            }
          } else if (game.get('status') === 'ended') {
            return this.show_tpl('status_ended', {
              result: data.game.result
            });
          }
        },
        call_finishing_ask_received: function(player) {
          if (game.is_player()) {
            return this.show_tpl('call_finishing_ask_received');
          } else {
            return this.show_tpl('call_finishing_ask_received_others', {
              player: player
            });
          }
        },
        show_analysis: function(regiment_list) {
          var regiment_tpl, _ref1, _ref2,
            _this = this;
          ...
          if (game.is_player()) {
            ...
            this.show_analysis_confirm();
            this.$el.find('#confirm').click(function() {
              return game.call_finishing_confirm(function(rlt) {
                if (rlt) {
                  return _this.show_tpl('status_ended', {
                    result: rlt
                  });
                } else {
                  _this.$el.find('#confirm').hide();
                  return _this.$el.find('#confirmed').show();
                }
              });
            });
            if ((_ref1 = game.get('last_action')) != null ? (_ref2 = _ref1.suggest_confirm) != null ? _ref2[game.my_role()] : void 0 : void 0) {
              return this.$el.find('#confirm').hide();
            } else {
              return this.$el.find('#confirmed').hide();
            }
          } else {
            return this.show_tpl('call_finishing_accept_received_others');
          }
        },
        show_analysis_confirm: function() {
          if (regiment_list.some(function(x) {
            ...
            return this.$el.find('#confirmed').show();
          } else {
        ...
        show_tpl: function(name, params) {
          ...
          if (this.find_tpl(name)) {
            this.$el.empty().append(this.find_tpl(name)(_.defaults(params != null ? params : {}, {
              data: data
            })));
          }
          switch (name) {
            case 'started_please_move':
              return $('#call-finishing').click(function() {
                return game.call_finishing_ask();
              });
            case 'started_please_wait':
              return $('#retract').click(function() {
                return game.retract();
              });
            case 'taking_seat':
              return $('.taking_seat a').click(function() {
                return game.taking_seat($(this).attr('_type'));
              });
            case 'call_finishing_ask':
              return this.$el.find('#call_finishing_cancel').click(function() {
                return game.call_finishing_cancel();
              });
            case 'call_finishing_ask_received':
        ...
        next_player: function() {
          ...
          if (arguments.length === 1) {
            ...
            model = arguments[0], player = arguments[1];
          }
          if (game.is_player()) {
            if (game.my_role() === player) {
              return this.show_tpl('started_please_move');
            } else {
              return this.show_tpl('started_please_wait');
            }
          } else {
            return this.show_tpl('next_player', {
              next: player
            });
          }
        },
        retract: function(stone) {
          if (game.is_player()) {
            if (game.my_role() === stone.player) {
              return this.show_tpl('started_please_move');
            } else {
              return this.show_tpl('retract_by_opponent');
            }
          } else {
            return console.log('retracted');
          }
        }
      });
        ...
        game.connect(function(err) {
          ...
          if (cf = game.get('calling_finishing')) {
            switch (cf.msg) {
              case 'ask':
                if (!game.is_player()) {
                  return bulletin.call_finishing_ask_received(game.role_of_player(cf.uid));
                } else if (cf.uid === game.myself()) {
                ...
                break;
              case 'accept':
                if (game.is_player() && cf.uid === game.myself()) {
